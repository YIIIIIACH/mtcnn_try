...
data_root = pathlib.Path('/home/yiiiiiach/MTCNN_TRY/image_align_celeba/')  
						#  打開  ./img_of_all.json 、image directory 將所有 landmark 與 bbox 標注 、圖片path 
...
path_ds = tf.data.Dataset.from_tensor_slices(all_image_paths)               
	#  tf.data.Dataset.from_tensor_slices(  <list>  ) ==> 將list 中的資料切成片段 以方便以類似串流的方式 丟進模型裡訓練
...
#  tf.data.Dataset.zip(( <Dataset> , <Dataset> , ........... ))   將Dataset 結合在一起 (要注意是否與 model 合適）
...
image_label_ds = image_label_ds.apply(tf.data.experimental.shuffle_and_repeat(buffer_size=1000))    
							# 將 image_label_ds 進行 1. 重排 2. batch化  等預處理
...
#宣告model 的輸入與輸出 對應到哪個 layer  , 其中 tf.keras.models.Model() 的參數 要與輸入輸出layer 的name 一樣
 #因為輸出為 regression 故使用 RMSoptimizer      
